...
public abstract class ContainerCloudletScheduler {
    ...
    /**
     * 这个方法不一定对，是直接从ContainerCloudletSchedulerJustToTry复制过来的
     */
    public double getRemainingTime() {
        ...
        for (ResCloudlet rcl : getCloudletExecList()) {
            remainTime = getEstimatedFinishTime(rcl, CloudSim.clock()) - CloudSim.clock();
        }
        for (ResCloudlet rcl : getCloudletWaitingList()) {
            remainTime += rcl.getRemainingCloudletLength() / getTotalCurrentAllocatedMipsForCloudlet(rcl, CloudSim.clock());
        }
        return remainTime;
    }
    /**
