 ...
 */
public class MyWFCExample3 {

    private static String experimentName = "MyWFCExampleStatic3";
...
//                        + indent + indent + indent + indent + datacenter.getVmAllocationPolicy().getHost(job.getVmId(), job.getUserId()).getId()
        ...
        // 记录成功数量和冗余成功数量，并统计每个工作流的完成时间
        for (Job job : list) {
            counter22[job.getCloudletId() - 1] = true;
            if (job.getStatus() == Cloudlet.SUCCESS) {
                double timeEqual = job.getFinishTime();
                Cloudlet master;
                if (WFCReplication.getSlaveToMasterMap().containsKey(job)) {
                    master = WFCReplication.getSlaveToMasterMap().get(job);
                    if (!failedJobIds.contains(master.getCloudletId())) {
                        redundant_success_counter++;
                    } else {
                        success_counter++;
                    }
                    if (master.getStatus() == Cloudlet.SUCCESS) {
                        timeEqual = Math.min(timeEqual, master.getFinishTime());
                    }
                } else {
                    master = job;
                    success_counter++;
                }
                // job可能对应一个集合，该集合中任何一个任务都是和job等效的
                // 只考虑job对应的这一层的即可，job会出现代表job以上的“层“全部失败，job会成功代表job之后没有其它”层“
                if(WFCReplication.getMasterToSlaveMap().containsKey(master)){
                    for (Cloudlet slave : WFCReplication.getMasterToSlaveMap().get(master)) {
                        if (slave.getStatus() == Cloudlet.SUCCESS) {
                            timeEqual = Math.min(timeEqual, slave.getFinishTime());
                        }
                    }
                }
                // 记录工作流的完成时间
                if (!WFCDeadline.finishTimeOfWorkflow.containsKey(job.getWorkflowId())
                        || WFCDeadline.finishTimeOfWorkflow.get(job.getWorkflowId()) < timeEqual) {
                    WFCDeadline.finishTimeOfWorkflow.put(job.getWorkflowId(), timeEqual);
                }
            }
        }
...
//        Log.printConcatLine("容器给WFCDatacenter返回了一共", WFCDeadline.containerSchedulerSendJobNum, "个任务");
    ...
    // 获取cloudlet所在的“层”和之后的所有“层”的所有等效任务
    // “层”是指cloudlet和cloudlet复制而来的所有任务，以及这些任务复制得到的任务的集合
    protected static Set<Cloudlet> getEqualCloudlets(Cloudlet cloudlet) {
        Set<Cloudlet> equalCloudlets = new HashSet<>();
        Cloudlet master;
        if (WFCReplication.getSlaveToMasterMap().containsKey(cloudlet)) {
            master = WFCReplication.getSlaveToMasterMap().get(cloudlet);
        } else {
            master = cloudlet;
        }
        equalCloudlets.add(master);
        equalCloudlets.addAll(WFCReplication.getMasterToSlaveMap().get(master));

        Set<Cloudlet> newCloudlets = new HashSet<>();
        for (Cloudlet cloudlet1 : equalCloudlets) {
            if (WFCReplication.getOldResubmitToNew().containsKey(cloudlet1)) {
                newCloudlets.addAll(getEqualCloudlets(WFCReplication.getOldResubmitToNew().get(cloudlet1)));
            }
        }
        equalCloudlets.addAll(newCloudlets);
        return equalCloudlets;
    }
}